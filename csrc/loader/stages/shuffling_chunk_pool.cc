...
ShufflingChunkPool::InitializeChunkSources() {
  ...
  for (auto& source : uninitialized_sources) {
    ...
    if (total_chunks >= chunk_pool_size_ && chunk_pool_size_ > 0) break;
  ...
  if (total_chunks == 0) {
    LOG(ERROR) << "ShufflingChunkPool initialization failed: "
                  "ShufflingChunkPool requires at least one chunk during "
                  "startup.";
    throw std::runtime_error(
  ...
  if (total_chunks < chunk_pool_size_ && !output_queue()->IsClosed()) {
    LOG(WARNING) << "ShufflingChunkPool startup chunk requirement not met: "
               << total_chunks.load() << " < " << chunk_pool_size_
               << ". Proceeding anyway (potentially reduced randomness).";
  }
